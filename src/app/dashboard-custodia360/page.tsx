...
export default function DashboardCustodia360() {
  ...
  // Función para generar PDF de informe de entidad
  const generarPDFInformeEntidad = (entidad: EntidadAdmin) => {
    import('jspdf').then(({ jsPDF }) => {
      const doc = new jsPDF('p', 'mm', 'a4')
      const pageWidth = doc.internal.pageSize.getWidth()
      ...
      doc.setFontSize(18)
      doc.setTextColor(0, 0, 0)
      doc.text(`INFORME COMPLETO - ${entidad.nombre}`, pageWidth / 2, yPosition, { align: 'center' })

      yPosition += 10
      doc.setFontSize(12)
      doc.setTextColor(100, 100, 100)
      doc.text(`Generado el ${formatDate(new Date().toISOString())}`, pageWidth / 2, yPosition, { align: 'center' })
      ...
      doc.setFontSize(11)
      doc.setTextColor(0, 0, 0)
      doc.text(`• Nombre: ${entidad.nombre}`, 25, yPosition)
      yPosition += 7
      doc.text(`• Email: ${entidad.email_contacto}`, 25, yPosition)
      yPosition += 7
      doc.text(`• Plan: ${entidad.plan}`, 25, yPosition)
      yPosition += 7
      doc.text(`• Estado: ${entidad.estado}`, 25, yPosition)
      yPosition += 7
      doc.text(`• Fecha contratación: ${formatDate(entidad.fecha_contratacion)}`, 25, yPosition)

      yPosition += 15
      doc.setFontSize(14)
      doc.setTextColor(59, 130, 246)
      doc.text('CUMPLIMIENTO LOPIVI', 20, yPosition)
      ...
      doc.setFontSize(8)
      doc.setTextColor(150, 150, 150)
      doc.text('Informe de Entidad - Custodia360 | Generado automáticamente', pageWidth / 2, 280, { align: 'center' })

      doc.save(`Informe_${entidad.nombre.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`)
    }).catch(error => {
      console.error('Error generando PDF:', error)
      alert('Error al generar PDF')
    })
  ...
  const generarPDFCertificado = (entidad: EntidadAdmin) => {
    import('jspdf').then(({ jsPDF }) => {
      ...
      doc.setFontSize(32)
      doc.setTextColor(34, 197, 94)
      doc.text('CERTIFICADO DE CUMPLIMIENTO LOPIVI', pageWidth / 2, yPosition, { align: 'center' })
      ...
      yPosition += 15
      doc.setFontSize(14)
      doc.text('Certifica el cumplimiento íntegro de la Ley Orgánica de Protección', pageWidth / 2, yPosition, { align: 'center' })
      yPosition += 8
      doc.text('Integral a la Infancia y la Adolescencia frente a la Violencia', pageWidth / 2, yPosition, { align: 'center' })
      ...
      doc.text(`Fecha de emisión: ${formatDate(new Date().toISOString())}`, pageWidth / 2, yPosition, { align: 'center' })
      yPosition += 6
      doc.text(`Válido hasta: ${formatDate(new Date(Date.now() + 365*24*60*60*1000).toISOString())}`, pageWidth / 2, yPosition, { align: 'center' })
      ...
      // Footer con nueva certificación
      doc.setFontSize(8)
      doc.setTextColor(100, 100, 100)
      doc.text('CERTIFICACIÓN CUSTODIA360 - Documento oficial del sistema de protección infantil', pageWidth / 2, pageHeight - 25, { align: 'center' })
