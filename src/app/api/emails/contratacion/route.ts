...
export async function POST(request: Request) {
  try {
    ...
    // ========================================
    // PASO 1: Crear entidad en Supabase
    // ========================================
    let entityId = null
    ...
    try {
      ...
      if (supabaseUrl && supabaseServiceKey && !supabaseUrl.includes('placeholder')) {
        ...
        console.log('💾 Creando entidad en Supabase...')

        // Crear entidad con los datos completos incluyendo emails
        const { data: entityData, error: entityError } = await supabase
          .from('entities')
          .insert([{
            nombre: nombreEntidad,
            cif: cif,
            sector_code: tipoEntidad || 'general',
            direccion: direccion,
            telefono: telefono,
            web: web,
            email_admin: emailAdministrativo,           // ← EMAIL ADMIN
            email_contratante: emailContratante,        // ← EMAIL CONTRATANTE
            kit_comunicacion_activo: kitComunicacion || false,  // Se activa si se contrata desde inicio
        ...
        if (entityError) {
          console.error('❌ Error creando entidad:', entityError)
        } else if (entityData) {
          entityId = entityData.id
          console.log('✅ Entidad creada con ID:', entityId)
          console.log('✅ Emails guardados - Admin:', emailAdministrativo, '| Contratante:', emailContratante)
        }
      } else {
        console.warn('⚠️ Supabase no configurado, saltando creación de entidad')
      }
    } catch (supabaseError) {
      console.error('⚠️ Error en Supabase:', supabaseError)
      // Continuar con los emails aunque falle Supabase
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        entityId: entityId,  // ← ID de la entidad creada en Supabase
