...
export default function InformeInspeccionPage() {
  ...
  const datosInforme = {
    ...
    delegados: {
      principal: {
        nombre: sessionData?.nombre || 'Delegado Principal',
        email: sessionData?.email || 'delegado@entidad.com',
  ...
  const generarInformePDF = () => {
    ...
    try {
      ...
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(0, 0, 0)
      doc.text(`Informe generado por: ${datosInforme.delegados.principal.nombre}`, 20, yPos)
      yPos += 7
      doc.text(`Cargo: Delegado Principal de Protección`, 20, yPos)
      yPos += 7
      ...
      const nuevoInforme = {
        ...
        generadoPor: datosInforme.delegados.principal.nombre
      }
