...
-- Políticas básicas (permitir todo para desarrollo, refinar en producción)
...
-- NOTA: Esto es un ejemplo básico, se debe completar con respuestas reales

INSERT INTO quiz_answers (question_id, text, is_correct)
SELECT id, 'Proteger integralmente a todos los menores frente a cualquier forma de violencia', true
FROM quiz_questions WHERE text = '¿Qué objetivo principal persigue la LOPIVI?' LIMIT 1;

INSERT INTO quiz_answers (question_id, text, is_correct)
SELECT id, 'Regular solo el maltrato físico', false
FROM quiz_questions WHERE text = '¿Qué objetivo principal persigue la LOPIVI?' LIMIT 1;

INSERT INTO quiz_answers (question_id, text, is_correct)
SELECT id, 'Sancionar a los padres negligentes', false
FROM quiz_questions WHERE text = '¿Qué objetivo principal persigue la LOPIVI?' LIMIT 1;

INSERT INTO quiz_answers (question_id, text, is_correct)
SELECT id, 'Controlar las actividades de las entidades', false
FROM quiz_questions WHERE text = '¿Qué objetivo principal persigue la LOPIVI?' LIMIT 1;

-- Para el resto de preguntas, crear respuestas genéricas (se deben personalizar)
DO $$
DECLARE
  q_record RECORD;
BEGIN
  FOR q_record IN SELECT id FROM quiz_questions WHERE id NOT IN (SELECT DISTINCT question_id FROM quiz_answers)
  LOOP
    INSERT INTO quiz_answers (question_id, text, is_correct) VALUES
    (q_record.id, 'Respuesta correcta (completar con contenido real)', true),
    (q_record.id, 'Opción incorrecta A (completar)', false),
    (q_record.id, 'Opción incorrecta B (completar)', false),
    (q_record.id, 'Opción incorrecta C (completar)', false);
  END LOOP;
END $$;
