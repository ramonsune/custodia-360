...
export default function BienvenidaDelegadoSuplentePage() {
  ...
  useEffect(() => {
    const verificarSesion = async () => {
      try {
        // Primero verificar si hay una sesión de formación LOPIVI (sistema nuevo)
        ...
        console.log('📋 Sesión formación LOPIVI:', sessionFormacion ? 'ENCONTRADA' : 'NO ENCONTRADA')
        console.log('📋 Sesión antigua:', sessionAntigua ? 'ENCONTRADA' : 'NO ENCONTRADA')

        if (sessionFormacion) {
          const userData = JSON.parse(sessionFormacion);
          // Verificar que sea delegado suplente
          if (userData.tipo !== 'suplente') {
            console.log('❌ No es delegado suplente');
            setLoading(false);
            router.push('/login-delegados?tipo=suplente');
            return;
          }
          setUsuario(userData);
          console.log('✅ Sesión cargada para delegado suplente:', userData.nombre);
        } else if (sessionAntigua) {
          const userData = JSON.parse(sessionAntigua);
          // Verificar que sea delegado suplente
          if (userData.tipo !== 'suplente') {
            console.log('❌ No es delegado suplente en sistema antiguo');
            setLoading(false);
            router.push('/login-delegados?tipo=suplente');
            return;
          }
          setUsuario(userData);
          console.log('✅ Sesión antigua cargada para delegado suplente:', userData.nombre);
        } else {
