const descargarCertificado = async () => {
  ...
  // Estado del registro de miembros
  const [miembrosRegistrados, setMiembrosRegistrados] = useState({
    total: 45,
    registrados: 12,
    formacionCompleta: 8,
  ...
  // Comprobar si hay configuración de registro
  const [registroConfigurado, setRegistroConfigurado] = useState(false)

  // NUEVO: Estado del canal LOPIVI
  const [canalLopivi, setCanalLopivi] = useState({
    configurado: false,
    tipo: '',
    contacto: '',
    horario: '',
  ...
  useEffect(() => {
    ...
    // NUEVO: Verificar estado del canal LOPIVI
    verificarEstadoCanalLopivi()
  }, [])

  const verificarEstadoCanalLopivi = () => {
    const canalConfig = localStorage.getItem('canal_lopivi_config')
    const fechaRegistro = localStorage.getItem('fecha_registro_entidad')

    if (canalConfig) {
      const config = JSON.parse(canalConfig)
      setCanalLopivi({
        configurado: true,
        ...config
      })
    } else if (fechaRegistro) {
      // Calcular días restantes (30 días desde registro)
      ...
      setCanalLopivi({
        ...
        emailsEnviados: parseInt(localStorage.getItem('canal_lopivi_emails') || '0')
      })

      // Enviar recordatorios automáticos
      programarRecordatorios(diasRestantes)
    }
  }

  const programarRecordatorios = (diasRestantes) => {
    const emailsEnviados = parseInt(localStorage.getItem('canal_lopivi_emails') || '0')

    // Recordatorios en días: 25, 15, 5, 1, 0 (después del registro)
    const diasRecordatorio = [25, 15, 5, 1, 0]
    const indiceRecordatorio = diasRecordatorio.findIndex(dia => diasRestantes <= dia)

    if (indiceRecordatorio !== -1 && emailsEnviados <= indiceRecordatorio) {
      enviarRecordatorioCanalLopivi(diasRestantes, indiceRecordatorio)
    }
  }

  const enviarRecordatorioCanalLopivi = async (diasRestantes, indice) => {
    try {
      console.log(`📧 Enviando recordatorio canal LOPIVI - ${diasRestantes} días restantes`)

      // Simular envío de email recordatorio al delegado
      const emailData = {
        delegado: sessionData?.nombre,
        entidad: sessionData?.entidad,
        diasRestantes,
        urgencia: diasRestantes <= 5 ? 'alta' : diasRestantes <= 15 ? 'media' : 'baja'
      }

      // En producción, aquí iría una llamada a API para enviar email
      await fetch('/api/recordatorios/canal-lopivi', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(emailData)
      }).catch(error => {
        console.warn('API recordatorio no disponible, usando modo simulado')
      })

      // Marcar email como enviado
      localStorage.setItem('canal_lopivi_emails', (indice + 1).toString())

      // Actualizar estado
      setCanalLopivi(prev => ({
        ...prev,
        emailsEnviados: indice + 1
      }))

      // Mostrar notificación visual en el dashboard
      if (diasRestantes <= 5) {
        // Mostrar alerta crítica
        console.log('🚨 ALERTA CRÍTICA: Configure el canal LOPIVI inmediatamente')
      }
  ...
  const irAConfiguracionCanal = () => {
    router.push('/configurar-canal-lopivi')
  }
